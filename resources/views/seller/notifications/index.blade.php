...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <a href="{{ route('seller.dashboard') }}" class="px-5 py-2.5 bg-gray-50 text-gray-700 font-bold rounded-xl hover:bg-gray-100 transition-colors flex items-center gap-2 border border-gray-200">
            ...
            <ul class="divide-y divide-gray-100">
                @foreach($notifications as $notification)
                    ...
                    @if(is_null($notification->read_at))
                        <form action="{{ route('seller.notifications.read', $notification->id) }}" method="POST" class="m-0 p-0 block w-full">
                            @csrf
                            @method('PATCH')
